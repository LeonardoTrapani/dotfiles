---
name: effect-ts-guidelines
description: Patterns to use Effect. Use when reviewing, or creating complicated things using Effect.ts
---
...
const fetchUser = (id: string) =>
  Effect.tryPromise({
    try: () => fetch(`/api/users/${id}`).then((r) => r.json()),
      ...
      ApiError.make({ endpoint: `/api/users/${id}`, statusCode: 500, error }),
  });
```
...
Run `effect-solutions show <topic>` for full docs.
