...
function isChangeRequestInfo(value: unknown): value is ChangeRequestInfo {
  if (!isRecord(value)) return false;

  return (
    (value.kind === "pull-request" || value.kind === "merge-request") &&
...
export function isGitInfoState(value: unknown): value is GitInfoState {
  ...
  return (
    ...
    (value.changeRequest === null || isChangeRequestInfo(value.changeRequest))
  );
}
