...
test(
  "Claude backend keeps context across follow-up turns",
  { timeout: 60_000 },
  async (t) => {
    ...
    try {
      ...
      const sessionFilePath = first?.meta.sessionFilePath;
      await runTool(
        runtime,
        manager.send(
          started.id,
          "What exact phrase did I ask you to reply with? Reply with only that phrase.",
        ),
      );
      await deadline(runTool(runtime, manager.waitFor([started.id])), 45_000);
      ...
      assert.match(second?.finalText ?? "", /hello claude/i);
      assert.equal(second?.meta.nativeSessionId, nativeSessionId);
      assert.equal(second?.meta.sessionFilePath, sessionFilePath);
    } finally {
