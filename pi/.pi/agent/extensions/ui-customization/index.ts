    ...
    ctx.ui.setFooter((tui, theme, footerData: ReadonlyFooterDataProvider) => {
      ...
      return {
        ...
        render(width: number) {
          ...
          if (gitInfo.changeRequest) {
            const kind =
              gitInfo.changeRequest.kind === "merge-request" ? "MR" : "PR";
            const requestLabel = `${kind} #${gitInfo.changeRequest.number}`;
            const linkedRequest = getCapabilities().hyperlinks
              ? hyperlink(requestLabel, gitInfo.changeRequest.url)
              : requestLabel;
            git += ` · ${linkedRequest}`;
          }
